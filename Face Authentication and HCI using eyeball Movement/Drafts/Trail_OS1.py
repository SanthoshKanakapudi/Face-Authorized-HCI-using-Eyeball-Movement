    ...
    if fc%5!=0:
        try:
            ...
            if best_sim > 0.8:  # Threshold (tune as needed)
                text, color = "You're Verified", (0, 255, 0)
                print(text)
                while True:
                    try: 
                        ret, frame = cap.read()
                        ...
                        if lanP:
                            ...
                            if abs(left_eye[0].y - left_eye[1].y) < 0.004:
                                ...
                                if current_time - lastbt < 0.3: #another blink
                                    blink_count += 1
                                else:
                                    blink_count = 1 
                            ...
                            if result.multi_hand_landmarks:
                                for hl in result.multi_hand_landmarks:
                                        ...
                                        if thumb_index_distance < 0.05:  #
                                            ...
                                            new_volume = max(0.0, current_volume - step_vol)  # Reduce volume
                                            volume.SetMasterVolumeLevelScalar(new_volume, None)
                                            print("Volume Decreased")
                                        
                                        elif thumb_middle_distance < 0.05:
                                            current_volume = volume.GetMasterVolumeLevelScalar()
                                            new_volume = min(1.0, current_volume + step_vol)  # Increase volume
                                            volume.SetMasterVolumeLevelScalar(new_volume, None)
